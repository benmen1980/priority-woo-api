...
class WooAPI extends \PriorityAPI\API
{
	...
	public function syncItemsPriority()
	{
		...
		$response = $this->makeRequest('GET', 'LOGPART?$filter=MPARTNAME eq \'\'    and ISMPART eq \'\' and ROYY_ISUDATE eq \'Y\' ', [], $this->option('log_items_priority', true));
		...
		if ($response['status']) {
			...
			foreach($response_data['value'] as $item) {
				...
				if ($id) {
					...
					update_post_meta($id, '_price', $item['VATPRICE']);
		...
		if ($response['status']) {
			...
			foreach ($parents as $partname => $value) {
				if (count($childrens[$partname])) {
					...
					$parents[$partname]['title']       = end($childrens[$partname])['parent_title'];
		...
		if ($response['status']) {
			...
			foreach($data['value'] as $item) {
				...
				if ($id = wc_get_product_id_by_sku($item['PARTNAME'])) {
					...
					// reset the parent stock status
					$var_parent = wc_get_product($parent_id);
					// For variable product types
					if($var_parent) {
						if ( $var_parent->is_type( 'variable' ) ) {
							$is_soldout = true;
							foreach ( $var_parent->get_available_variations() as $variation ) {
		...
		foreach ($order->get_items() as $item) {
			...
			if ($product) {
				...
				if ($items) {
					...
					foreach ($items as $item_id => $item_for_meta) {
